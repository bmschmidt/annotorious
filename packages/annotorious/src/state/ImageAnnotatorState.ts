...
export const createImageAnnotatorState = <
  I extends Annotation,
  E extends unknown
>(
  ...
  const selection = createSelectionState<I, E>(
    store,
    opts.userSelectAction,
    opts.adapter
  );
  ...
  store.observe(({ changes }) => {
    tree.set(
      (changes.created || []).map((a) => a.target as ImageAnnotationTarget),
      false
    );

    (changes.deleted || []).forEach((a) =>
      tree.remove(a.target as ImageAnnotationTarget)
    );
  ...
  const getAt = (x: number, y: number): ImageAnnotation | undefined => {
    ...
    return target
      ? (store.getAnnotation(target.annotation) as unknown as ImageAnnotation)
      : undefined;
  };
  ...
  const getIntersecting = (
    ...
    tree
      .getIntersecting(x, y, width, height)
      .map(
        (target) =>
          store.getAnnotation(target.annotation) as unknown as ImageAnnotation
      );
...
export const createSvelteImageAnnotatorState = <
  I extends Annotation,
  E extends unknown
>(
  opts: AnnotoriousOpts<I, E>
