...
export type W3CImageFormatAdapter = FormatAdapter<
  ImageAnnotation,
  W3CImageAnnotation
>;
...
export const parseW3CImageAnnotation = (
  ...
  const { creator, created, modified, body, ...rest } = annotation;
  ...
  const selector =
    w3cSelector?.type === 'FragmentSelector'
      ...
      : w3cSelector?.type === 'SvgSelector'
        ? parseSVGSelector(w3cSelector as SVGSelector)
        : undefined;
...
export const serializeW3CImageAnnotation = (
  ...
  if ('annotation' in serialized.target) delete serialized.target.annotation;
